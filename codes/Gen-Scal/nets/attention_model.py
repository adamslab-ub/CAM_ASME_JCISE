...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.robot_taking_decision_query = nn.Linear(4, embedding_dim)
    ...
    def _inner_eval(self, input, embeddings):
        ...
        while not (self.shrink_size is None and not (state.all_finished().item() == 0)):
            ...
            # log_p, mask = self._get_log_p_nn(fixed, state) # used for decoder ablation
    ...
    def _get_parallel_step_context(self, embeddings, state, from_depot=False):
        ...
        current_robot_states = torch.cat(
            (
                state.coords[state.ids,robots_current_destination],
                state.robots_range_remaining[:,:,None],
                state.robots_capacity[:,:, None],
                state.robots_next_decision_time[:,:, None]
            ),
            -1)
        decision_robot_state = torch.cat((state.coords[state.ids, state.robots_current_destination[state.ids, state.robot_taking_decision]].view(batch_size,-1),
                                          state.robot_taking_decision_range,
                                          state.robots_capacity[state.ids,state.robot_taking_decision]),-1) # add depot info here??
